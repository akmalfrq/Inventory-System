...
class Barang extends CI_Controller {
	...
	public function transaksi_masuk()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$kode = 'B'.date('y').'-'.substr($barang, 1).substr($kategori, 1).'-';
			$kodemax = $this->masterm->find_kode_master('kd_barang', 'barang', -3, $kode);
			...
			for ($i=0; $i < $jumlah; $i++) { 
				$kd_barang = $kode. str_pad($kodemax, 3, '0', STR_PAD_LEFT);
